...
# OpenAI Configuration (v1+ format)
...
try:
    ...
    from .schemas import (
        JobCreate, JobRead, JobUpdate, JobSummary,
        UserCreate, UserLogin, UserRead, Token,
        SalarySuggestionRequest, SalarySuggestionResponse, 
        ExperienceLevelRequest, ExperienceLevelResponse, 
        AutoCompleteRequest, AutoCompleteSuggestion, AutoCompleteResponse
    )
    from .crud import (
        create_job, get_jobs, get_job, get_user_jobs, get_user_job, 
        update_job, delete_job, publish_job,
    ...
    from .ai_service import (
        generate_ai_summary, analyze_ai_requirements, generate_ai_suggestions,
        is_ai_enabled, get_ai_status
    )
except ImportError:
...
def normalize_job_title_v3_enhanced(input_title: str, description: str = "") -> Dict[str, Any]:
    ...
    # Skip normalization for test jobs to preserve original titles during testing
    if input_clean.startswith("test ") or input_clean == "test" or "test job" in input_clean:
        return {
            "original_title": input_title,
            "normalized_title": input_title,
            "confidence": 100.0,
            "suggestions": [],
            "skills_analysis": None,
            "ai_disclaimer": "⚠️ Test job detected - normalization skipped to preserve original title.",
            "match_quality": "test_job"
        }
...
@app.get("/users/me/jobs", response_model=List[JobSummary])
def get_my_jobs(
    skip: int = 0, 
    limit: int = 100,
...
@app.post("/users/me/jobs", response_model=JobRead)
def create_my_job(
    job: JobCreate,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
    ...
    try:
        logger.info(f"Creating job for user {current_user.id}: {job.title}")
        logger.info(f"Job data: {job.dict()}")
        created_job = create_job(db, job, current_user.id)
        logger.info(f"Successfully created job with ID: {created_job.id}")
        return created_job
    except Exception as e:
        logger.error(f"Error in create_my_job: {str(e)}")
        logger.error(f"Job data that failed: {job.dict() if job else 'None'}")
        raise HTTPException(status_code=500, detail=f"Failed to create job: {str(e)}")

@app.get("/users/me/jobs/{job_id}", response_model=JobRead)
def get_my_job(
    job_id: int,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """Get a specific job for the current user"""
    job = get_user_job(db, job_id, current_user.id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return job

@app.put("/users/me/jobs/{job_id}", response_model=JobRead)
def update_my_job(
    job_id: int,
    ...
    job = update_job(db, job_id, current_user.id, job_update)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return job

@app.delete("/users/me/jobs/{job_id}")
def delete_my_job(
    job_id: int,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
    ...
    success = delete_job(db, job_id, current_user.id)
    if not success:
        raise HTTPException(status_code=404, detail="Job not found")
    return {"message": "Job deleted successfully"}

@app.post("/users/me/jobs/{job_id}/publish", response_model=JobRead)
def publish_my_job(
    job_id: int,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """Publish a job for the current user"""
    job = publish_job(db, job_id, current_user.id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return job
...
def seed_jobs(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for job_data in jobs_data:
            job = create_job(db, JobCreate(**job_data), user_id)
            seeded.append(job)
            logger.info(f"Seeded job: {job.title}")
        
        return {"message": f"Successfully seeded {len(seeded)} jobs", "jobs": seeded, "user_id": user_id}
    
    except Exception as e:
        logger.error(f"Error seeding jobs: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to seed jobs: {str(e)}")

# Simple test job creation endpoint
@app.post("/jobs/test", response_model=JobRead)
def create_test_job(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    """Create a simple test job with minimal data to save tokens during testing"""
    
    test_job_data = JobCreate(
        title="Test Developer",
        description="Simple test job for development testing. Basic programming skills required.",
        requirements=[
            {"skill": "Programming", "level": "Basic", "weight": 0.8},
            {"skill": "Problem Solving", "level": "Intermediate", "weight": 0.7}
        ],
        seniority="Junior",
        domain_tags=["tech", "testing"],
        screening_questions=["Can you code?", "Do you like testing?"],
        ats_keywords=["Programming", "Testing", "Development"],
        summary="Test job for development purposes",
        status="draft"
    )
    
    try:
        job = create_job(db, test_job_data, current_user.id)
        logger.info(f"Created test job: {job.title} for user {current_user.username}")
        return job
    except Exception as e:
...
async def call_openai_gpt(prompt: str, max_tokens: int = 2000, temperature: float = 0.7) -> str:
    """Make a call to OpenAI GPT API with fallback to rule-based system"""
    try:
        if not AI_ENABLED or not client.api_key:
            logger.warning("OpenAI API disabled or key missing, using fallback")
            return None
            
        response = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[
                {"role": "system", "content": "You are an expert HR professional and job description analyst. Provide clear, professional, and actionable responses."},
...
async def generate_gpt_summary(job_data: dict, style: str = "standard") -> str:
    """Generate job summary using GPT with fallback to rule-based system"""
    try:
        ...
        req_text = ""
        if requirements:
            req_text = "\n".join([f"- {req}" if isinstance(req, str) else f"- {req.get('skill', 'N/A')} ({req.get('level', 'Intermediate')})" for req in requirements])
        
        if style == "ats":
            prompt = f"""
Create an ATS-optimized job summary for:
Title: {title}
Seniority: {seniority}
Description: {description}
Requirements: {req_text}

Make it keyword-rich and ATS-friendly while maintaining readability. Include relevant industry keywords and format for applicant tracking systems.
"""
        elif style == "short":
            prompt = f"""
Create a concise, engaging job summary (150 words max) for:
Title: {title}
Seniority: {seniority}
Description: {description}
Requirements: {req_text}

Focus on the most compelling aspects and key requirements. Make it attention-grabbing for job seekers.
"""
        else:  # standard
            prompt = f"""
Create a comprehensive, professional job summary for:
Title: {title}
Seniority: {seniority}
Description: {description}
Requirements: {req_text}

Include role overview, key responsibilities, requirements, and what makes this opportunity attractive. Format with clear sections and bullet points.
"""
        
        gpt_response = await call_openai_gpt(prompt)
        
        if gpt_response:
            logger.info(f"Generated GPT {style} summary for: {title}")
            return gpt_response
        else:
            # Fallback to original rule-based system
            logger.info(f"Using fallback rule-based summary for: {title}")
            return generate_summary_ai(job_data, style)
...
async def analyze_requirements_gpt(title: str, description: str) -> List[Dict]:
    """Analyze job requirements using GPT with intelligent skill extraction"""
    try:
        prompt = f"""
...
Return a JSON array of requirements with this format:
[
  {{"skill": "Python", "level": "Advanced", "category": "core", "importance": "high"}},
  {{"skill": "Communication", "level": "Intermediate", "category": "soft", "importance": "medium"}}
]

Categories: core, bonus, soft
Levels: Beginner, Intermediate, Advanced, Expert
Importance: low, medium, high

Extract 8-15 requirements covering technical skills, soft skills, and tools/frameworks.
"""
        
        gpt_response = await call_openai_gpt(prompt, max_tokens=1500)
        
        if gpt_response:
            try:
                # Parse JSON response
                requirements = json.loads(gpt_response)
                logger.info(f"Generated GPT requirements for: {title} ({len(requirements)} requirements)")
                return requirements
            except json.JSONDecodeError:
                logger.warning("GPT response was not valid JSON, using fallback")
                
        # Fallback to rule-based system
        logger.info(f"Using fallback rule-based requirements for: {title}")
        return analyze_requirements_ai(title, description)
        
    except Exception as e:
        logger.error(f"Error analyzing requirements with GPT: {str(e)}")
        if AI_FALLBACK_ENABLED:
            return analyze_requirements_ai(title, description)
        else:
            raise

async def generate_gpt_screening_questions(title: str, description: str, requirements: List) -> List[str]:
    """Generate screening questions using GPT"""
    try:
        req_text = "\n".join([f"- {req}" if isinstance(req, str) else f"- {req.get('skill', 'N/A')}" for req in requirements[:10]])
        
        prompt = f"""
Generate 4-6 screening questions for this job:
Title: {title}
Description: {description}
Key Requirements: {req_text}

Create questions that:
1. Test technical competency
2. Assess cultural fit
3. Evaluate experience level
4. Check problem-solving skills

Return as a simple list, one question per line.
"""
        
        gpt_response = await call_openai_gpt(prompt, max_tokens=800)
        
        if gpt_response:
            questions = [q.strip() for q in gpt_response.split('\n') if q.strip() and not q.strip().startswith('#')]
            questions = [q.lstrip('0123456789.- ') for q in questions]  # Clean numbering
            logger.info(f"Generated GPT screening questions for: {title} ({len(questions)} questions)")
            return questions[:6]  # Limit to 6 questions
            
        # Simple fallback
        return [
            f"What experience do you have with the key technologies mentioned in this {title} role?",
            "Describe a challenging project you've worked on and how you overcame obstacles.",
            "How do you stay updated with industry trends and best practices?",
            "What motivates you to apply for this position specifically?"
        ]
        
    except Exception as e:
        logger.error(f"Error generating GPT screening questions: {str(e)}")
        return [
...
async def generate_gpt_smart_skills(title: str, description: str = "") -> List[Dict]:
    """Generate smart skills suggestions using GPT with contextual analysis"""
    try:
        ...
        prompt = f"""
Analyze this job posting and suggest the most relevant skills:
{context}

Provide a JSON array of smart skills suggestions with this format:
[
  {{"skill": "Python", "category": "core", "priority": "high", "reasoning": "Primary programming language for backend development"}},
  {{"skill": "Docker", "category": "tools", "priority": "medium", "reasoning": "Container technology for deployment"}}
]

Categories: core, bonus, tools, soft, frameworks
Priority: high, medium, low

Focus on:
1. The most relevant technical skills for this specific role
2. Industry-standard tools and frameworks
3. Important soft skills for the position
4. Modern technologies that would enhance the role

Return 10-15 diverse skills covering technical and soft skills.
"""
        ...
        if gpt_response:
            try:
                skills = json.loads(gpt_response)
                logger.info(f"Generated GPT smart skills for: {title} ({len(skills)} skills)")
                return skills
            except json.JSONDecodeError:
                logger.warning("GPT response was not valid JSON for smart skills")
                
        # Fallback to rule-based system
        logger.info(f"Using fallback smart skills for: {title}")
        normalization = normalize_job_title_v3_enhanced(title, description)
        fallback_skills = []
        if normalization.get("skills_analysis"):
            suggested = normalization["skills_analysis"].get("suggested_skills", [])
            fallback_skills = [
                {
                    "skill": skill,
                    "category": "core" if i < len(suggested)//2 else "bonus",
                    "priority": "high" if i < 3 else "medium" if i < 8 else "low",
                    "reasoning": f"Pattern-matched suggestion based on job title analysis"
                }
                for i, skill in enumerate(suggested)
            ]
        return fallback_skills
        
    except Exception as e:
        logger.error(f"Error generating GPT smart skills: {str(e)}")
        return []

async def generate_gpt_autocomplete(context: str, field_type: str, job_title: str = "") -> List[str]:
    """Generate intelligent auto-complete suggestions using GPT"""
    try:
        if field_type == "description":
            prompt = f"""
Complete this job description text professionally:
Job Title: {job_title}
Current Text: {context}

Provide 5-8 natural continuations that would complete the current sentence or add relevant next sentences. Each suggestion should:
1. Flow naturally from the existing text
2. Be specific and actionable
3. Use professional language
4. Be relevant to the job title
...
"""
            ...
            prompt = f"""
Complete this job requirements text:
Job Title: {job_title}
Current Text: {context}

Provide 5-8 relevant requirement completions or additions. Focus on:
...
"""
            ...
            prompt = f"""
Complete this job summary text:
Job Title: {job_title}
Current Text: {context}

Provide 5-8 professional summary completions that:
1. Highlight key aspects of the role
2. Mention growth opportunities
3. Describe company culture or benefits
4. Use engaging, recruitment-focused language

Return as a JSON array of strings:
["summary point 1", "summary point 2", ...]
"""
        
        gpt_response = await call_openai_gpt(prompt, max_tokens=800, temperature=0.8)
        
        if gpt_response:
            try:
                suggestions = json.loads(gpt_response)
                logger.info(f"Generated GPT autocomplete for {field_type}: {len(suggestions)} suggestions")
                return suggestions[:8]  # Limit to 8 suggestions
            except json.JSONDecodeError:
                logger.warning("GPT response was not valid JSON for autocomplete")
                
        return []
        
    except Exception as e:
        logger.error(f"Error generating GPT autocomplete: {str(e)}")
        return []

def convert_gpt_to_hr_structure(gpt_analysis: List[Dict], title: str, description: str) -> Dict:
    """Convert GPT analysis to HR-compatible structure"""
    try:
        core_skills = []
        soft_skills = []
        bonus_skills = []
        
        for item in gpt_analysis:
            skill_data = {
                "skill": item.get("skill", ""),
                "level": item.get("level", "Intermediate"),
                "rating": 4 if item.get("importance", "medium") == "high" else 3 if item.get("importance", "medium") == "medium" else 2
            }
            
            category = item.get("category", "core").lower()
            if category == "core":
                core_skills.append(skill_data)
            elif category == "soft":
                soft_skills.append(skill_data)
            else:
                bonus_skills.append(skill_data)
        ...
        return {
            ...
            "soft_skills": soft_skills,
            "bonus_skills": bonus_skills,
            "experience": {"level": seniority, "years": "3-5" if seniority == "Mid" else "5+" if seniority == "Senior" else "1-3"},
            "languages": [],
            "tracks": {},
            "metadata": {
                "seniority": seniority,
                "job_types": [title.lower()],
                "is_multi_track": False,
                "track_count": 1
            }
        }
    except Exception as e:
        logger.error(f"Error converting GPT analysis to HR structure: {e}")
        # Return empty structure on error
        return {
            "core_skills": [],
            "soft_skills": [],
            "bonus_skills": [],
            "experience": {"level": "Mid", "years": "2-4"},
            "languages": [],
            "tracks": {},
            "metadata": {"seniority": "Mid", "job_types": [], "is_multi_track": False, "track_count": 0}
        }

async def normalize_title_with_gpt(title: str, description: str = "") -> Dict:
    """Normalize job title using GPT with intelligent suggestions"""
    try:
        context = f"Job Title: {title}"
        if description:
            context += f"\nDescription: {description}"
        ...
        prompt = f"""
Analyze and normalize this job title:
{context}

Provide a JSON response with:
{{
  "normalized_title": "Standardized professional title",
  "confidence": 0.95,
  "corrections_made": ["Fixed spelling", "Standardized format"],
  "suggestions": [
    {{"title": "Alternative 1", "confidence": 0.85, "reason": "More specific"}},
    {{"title": "Alternative 2", "confidence": 0.80, "reason": "Industry standard"}}
  ],
  "key_technologies": ["Python", "React", "AWS"],
  "seniority": "Senior|Mid|Junior",
  "category": "Engineering|Marketing|Sales|Design|Data"
}}

Focus on:
1. Correcting typos and formatting
2. Using industry-standard terminology  
3. Maintaining the core role intent
4. Extracting relevant technologies from description
"""
        ...
        if gpt_response:
            try:
                ...
                logger.info(f"GPT normalized title: '{title}' → '{result.get('normalized_title')}'")
                return result
            except json.JSONDecodeError:
                logger.warning("GPT response was not valid JSON for title normalization")
                
        return None
        
    except Exception as e:
        logger.error(f"Error normalizing title with GPT: {str(e)}")
        return None

# ===== END OPENAI GPT INTEGRATION =====
...
async def suggest_requirements(job_id: int, db: Session = Depends(get_db)):
    """Step 2: AI-powered requirements suggestion"""
    try:
        ...
        gpt_analysis = await analyze_requirements_gpt(job.title, job.description)
        
        if gpt_analysis:
            # Convert GPT format to legacy format for database
            suggested_requirements = []
            for req in gpt_analysis:
                if isinstance(req, dict):
                    suggested_requirements.append(f"{req.get('skill', 'Unknown')}:{req.get('level', 'Intermediate')}")
                else:
                    suggested_requirements.append(str(req))
            logger.info(f"Generated AI requirements for job {job_id}: {len(suggested_requirements)} requirements")
        else:
            # Fallback to rule-based analysis
            suggested_requirements = analyze_requirements_ai(job.title, job.description)
            logger.info(f"Generated rule-based requirements for job {job_id}: {len(suggested_requirements)} requirements")
...
# CRUD Endpoints (Public and Legacy)
@app.get("/jobs", response_model=List[JobRead])
def read_jobs(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Get list of all published jobs (public endpoint)"""
    try:
        jobs = get_jobs(db, skip, limit)
        logger.info(f"Retrieved {len(jobs)} published jobs")
        return jobs
    except Exception as e:
...
def create_new_job(
    ...
    try:
        created_job = create_job(db, job, current_user.id)
        logger.info(f"Created new job: {created_job.title} for user {current_user.id}")
        return created_job
    except Exception as e:
...
def read_job(job_id: int, db: Session = Depends(get_db)):
    """Get a specific job by ID (public endpoint for published jobs)"""
    try:
        job = get_job(db, job_id)
        if not job:
            raise HTTPException(status_code=404, detail="Job not found")
        
        # Only show published jobs on public endpoint
...
async def analyze_requirements_endpoint(request: dict):
    """Analyze job title and description with AI-first HR-focused categorization"""
    try:
        ...
        # Try AI analysis first, fallback to rule-based
        gpt_analysis = await analyze_requirements_gpt(title, description)
        
        if gpt_analysis:
            # Convert GPT analysis to HR structure
            analysis = convert_gpt_to_hr_structure(gpt_analysis, title, description)
            ai_powered = True
            ai_status = "GPT-Enabled"
        else:
            # Fallback to rule-based analysis
            analysis = analyze_requirements_ai(title, description)
            ai_powered = False
            ai_status = "Rule-Based"
        ...
        logger.info(f"Analyzed requirements for '{title}' using {ai_status}: {len(legacy_requirements)} total requirements found")
...
async def hr_analysis_endpoint(request: dict):
    """Dedicated HR analysis endpoint with professional reporting format using GPT"""
    try:
        title = request.get('title', '')
        description = request.get('description', '')
        ...
        # Use the new GPT service for enhanced analysis
        analysis = await analyze_requirements_gpt(title, description)
        
        # Generate screening questions using GPT
        screening_questions = await generate_gpt_screening_questions(title, description, analysis)
        ...
        return {
            ...
            "ai_powered": AI_ENABLED and client.api_key is not None,
            "ai_status": "GPT-Enabled" if (AI_ENABLED and client.api_key) else "Rule-Based"
        }
...
async def generate_summary_endpoint(request: dict):
    """Generate AI-powered job summary with multiple style options using GPT"""
    try:
        ...
        # Use the new GPT integration for enhanced summary generation
        summary = await generate_gpt_summary(request, style)
...
async def normalize_job_title_v3_endpoint(request: dict):
    """
    V3 Enhanced Feature: AI-powered job title normalization with context-aware skills management
    
    Capabilities:
    - AI-powered smart corrections: "fronend enginer" → "Frontend Engineer"  
    ...
    try:
        ...
        gpt_result = await normalize_title_with_gpt(title, description)
        
        if gpt_result:
            # Use AI result
            result = {
                "original_title": title,
                "normalized_title": gpt_result.get("normalized_title", title),
                "confidence": gpt_result.get("confidence", 0.9),
                "match_quality": "high" if gpt_result.get("confidence", 0) > 0.85 else "medium",
                "suggestions": gpt_result.get("suggestions", []),
                "corrections_made": gpt_result.get("corrections_made", []),
                "key_technologies": gpt_result.get("key_technologies", []),
                "seniority": gpt_result.get("seniority", "Mid"),
                "category": gpt_result.get("category", "General")
            }
            ai_powered = True
            ai_status = "GPT-Enabled"
        else:
        ...
        enhanced_result = {
            **result,
            "api_version": "v4_ai_enhanced",
            "feature": "ai_powered_job_title_normalization",
            "ai_powered": ai_powered,
            "ai_status": ai_status,
            "processing_info": {
                "algorithm": "gpt_with_fallback_to_fuzzy_matching",
...
async def smart_skills_suggestions_endpoint(request: dict):
    """
    Enhanced V3 Feature: Get smart skills suggestions using GPT analysis
    """
    try:
        title = request.get("title", "").strip()
        description = request.get("description", "").strip()
        ...
        # Use GPT service for smart suggestions
        ai_suggestions = await generate_gpt_smart_skills(title, description)
        
        # Also get fallback suggestions for comparison
        normalization = normalize_job_title_v3_enhanced(title, description)
        fallback_skills = []
        if normalization.get("skills_analysis"):
            fallback_skills = normalization["skills_analysis"].get("suggested_skills", [])
        
        return {
            "api_version": "v4_gpt_enhanced",
            "feature": "smart_skills_suggestions",
            "ai_powered": AI_ENABLED and client.api_key is not None,
            "title_normalization": {
                "original": title,
                "normalized": normalization.get("normalized_title", title),
                "confidence": normalization.get("confidence", 0.8)
            },
            "skills_suggestions": {
                "ai_suggested_skills": ai_suggestions,
                "fallback_skills": fallback_skills,
                "total_suggestions": len(ai_suggestions),
                "ai_analysis": "Generated using GPT-4" if (AI_ENABLED and client.api_key) else "Using rule-based analysis"
            },
            "usage_guide": {
                "ai_skills": "🤖 AI-generated skills - Based on comprehensive GPT job analysis",
                "fallback_skills": "🔧 Rule-based skills - Pattern-matched suggestions", 
                "review_note": "📝 Always review AI suggestions for accuracy and relevance",
                "priority": "✨ AI suggestions are prioritized when available"
            },
            "ai_status": "GPT-Enabled" if (AI_ENABLED and client.api_key) else "Rule-Based",
            "ai_disclaimer": "⚠️ AI makes mistakes! Please review all skills and suggestions carefully for best results."
        }
...
# ============================================================================
# SALARY SUGGESTIONS ENDPOINT - Based on Skills + Location + Seniority
# ============================================================================

# Salary data structures - In production, this would come from external APIs
SALARY_BASE_DATA = {
    # Base salaries by job title and seniority (USD, annual)
    "Frontend Engineer": {
        "Junior": {"min": 65000, "max": 85000, "median": 75000},
        "Mid": {"min": 85000, "max": 120000, "median": 100000},
        "Senior": {"min": 120000, "max": 160000, "median": 140000},
        "Staff": {"min": 160000, "max": 200000, "median": 180000},
        "Principal": {"min": 200000, "max": 280000, "median": 240000}
    },
    "Backend Engineer": {
        "Junior": {"min": 70000, "max": 90000, "median": 80000},
        "Mid": {"min": 90000, "max": 130000, "median": 110000},
        "Senior": {"min": 130000, "max": 170000, "median": 150000},
        "Staff": {"min": 170000, "max": 220000, "median": 195000},
        "Principal": {"min": 220000, "max": 300000, "median": 260000}
    },
    "Full Stack Engineer": {
        "Junior": {"min": 68000, "max": 88000, "median": 78000},
        "Mid": {"min": 88000, "max": 125000, "median": 105000},
        "Senior": {"min": 125000, "max": 165000, "median": 145000},
        "Staff": {"min": 165000, "max": 210000, "median": 187500},
        "Principal": {"min": 210000, "max": 290000, "median": 250000}
    },
    "DevOps Engineer": {
        "Junior": {"min": 72000, "max": 92000, "median": 82000},
        "Mid": {"min": 95000, "max": 135000, "median": 115000},
        "Senior": {"min": 135000, "max": 180000, "median": 157500},
        "Staff": {"min": 180000, "max": 230000, "median": 205000},
        "Principal": {"min": 230000, "max": 320000, "median": 275000}
    },
    "Data Engineer": {
        "Junior": {"min": 75000, "max": 95000, "median": 85000},
        "Mid": {"min": 100000, "max": 140000, "median": 120000},
        "Senior": {"min": 140000, "max": 185000, "median": 162500},
        "Staff": {"min": 185000, "max": 240000, "median": 212500},
        "Principal": {"min": 240000, "max": 330000, "median": 285000}
    }
}
...
def normalize_job_title_for_salary(title: str) -> str:
    """Normalize job title for salary lookup"""
    title_lower = title.lower()
    
    # Frontend variations
    if any(term in title_lower for term in ["frontend", "front-end", "front end", "ui", "client"]):
        return "Frontend Engineer"
    
    # Backend variations
    if any(term in title_lower for term in ["backend", "back-end", "back end", "server", "api"]):
        return "Backend Engineer"
    
    # Full stack variations
    if any(term in title_lower for term in ["full stack", "fullstack", "full-stack"]):
        return "Full Stack Engineer"
    
    # DevOps variations
    if any(term in title_lower for term in ["devops", "sre", "platform", "infrastructure", "cloud"]):
        return "DevOps Engineer"
    
    # Data variations
    if any(term in title_lower for term in ["data engineer", "data scientist", "ml engineer", "ai engineer"]):
        return "Data Engineer"
    
    # Default to Full Stack for generic titles
    return "Full Stack Engineer"

def calculate_skills_premium(skills: List[str]) -> Dict[str, float]:
    """Calculate premium based on skills"""
    premiums = {}
    total_premium = 0.0
    
    for skill in skills:
        if skill in SKILLS_PREMIUMS:
            premium = SKILLS_PREMIUMS[skill]
            premiums[skill] = premium
            total_premium += premium
    
    # Cap total premium at 50%
    if total_premium > 0.5:
        scale_factor = 0.5 / total_premium
        premiums = {k: v * scale_factor for k, v in premiums.items()}
        total_premium = 0.5
    
    return {
        "individual_premiums": premiums,
        "total_premium": total_premium,
        "capped": total_premium >= 0.5
    }

def calculate_company_size_adjustment(company_size: str) -> float:
    """Calculate adjustment based on company size"""
    adjustments = {
        "startup": 0.85,    # Lower base, higher equity
        "small": 0.90,      # 10-100 employees
        "medium": 1.0,      # 100-1000 employees (baseline)
        "large": 1.15,      # 1000-10000 employees
        "enterprise": 1.25  # 10000+ employees
    }
    return adjustments.get(company_size.lower(), 1.0)

@app.post("/salary-suggestions", response_model=SalarySuggestionResponse)
async def get_salary_suggestions(request: SalarySuggestionRequest):
    """
    Generate salary suggestions based on job title, skills, location, and seniority.
    
    This endpoint provides comprehensive salary analysis including:
    - Base salary ranges by seniority level
    - Location-based adjustments
    - Skills-based premiums
    - Total compensation estimates
    - Market confidence indicators
    """
    try:
        # Normalize job title for salary lookup
        normalized_title = normalize_job_title_for_salary(request.job_title)
        
        # Get base salary data
        if normalized_title not in SALARY_BASE_DATA:
            raise HTTPException(status_code=400, detail=f"Salary data not available for job title: {normalized_title}")
        
        # Map seniority_level to the expected format
        seniority_mapping = {
            "Lead": "Staff",
            "Senior": "Senior", 
            "Mid": "Mid",
        ...
        mapped_seniority = seniority_mapping.get(request.seniority_level, "Mid")
        
        if mapped_seniority not in SALARY_BASE_DATA[normalized_title]:
            raise HTTPException(status_code=400, detail=f"Seniority level '{mapped_seniority}' not supported")
        
        base_salary_data = SALARY_BASE_DATA[normalized_title][mapped_seniority]
        
        # Calculate location adjustment
        location_adjustment = LOCATION_ADJUSTMENTS.get(request.location, 0.90)
        
        # Extract skill names from skill objects
        skill_names = [skill.skill for skill in request.skills]
        
        # Calculate skills premium
        skills_analysis = calculate_skills_premium(skill_names)
        
        # Calculate company size adjustment
        company_adjustment = calculate_company_size_adjustment(request.company_size or "medium")
        
        # Apply all adjustments to base salary
        total_multiplier = location_adjustment * (1 + skills_analysis["total_premium"]) * company_adjustment
        
        adjusted_salary = {
            "min": int(base_salary_data["min"] * total_multiplier),
            "max": int(base_salary_data["max"] * total_multiplier),
            "median": int(base_salary_data["median"] * total_multiplier)
        }
        ...
        if request.job_title.lower() == normalized_title.lower():
            confidence_factors.append(0.3)
        else:
            confidence_factors.append(0.2)
        
        # Location data confidence
        if request.location in LOCATION_ADJUSTMENTS:
            confidence_factors.append(0.25)
        else:
            confidence_factors.append(0.1)
        
        # Skills data confidence
        recognized_skills = len([s for s in skill_names if s in SKILLS_PREMIUMS])
        skills_confidence = min(0.25, recognized_skills * 0.05)
        ...
        # Create comparable positions
        comparable_positions = [
            {
                "title": f"{mapped_seniority} Frontend Developer",
                "company_size": "Large",
                "salary_range": f"${adjusted_salary['min']:,} - ${adjusted_salary['max']:,}",
                "location": request.location
            },
            {
                "title": f"{mapped_seniority} Full Stack Developer", 
                "company_size": "Medium",
                "salary_range": f"${int(adjusted_salary['min'] * 0.95):,} - ${int(adjusted_salary['max'] * 0.95):,}",
                "location": request.location
            },
            {
                "title": f"{mapped_seniority} Software Engineer",
                "company_size": "Startup", 
                "salary_range": f"${int(adjusted_salary['min'] * 0.85):,} - ${int(adjusted_salary['max'] * 0.85):,}",
                "location": request.location
            }
        ]
        ...
        return SalarySuggestionResponse(
            min_salary=adjusted_salary["min"],
            max_salary=adjusted_salary["max"], 
            average_salary=adjusted_salary["median"],
            percentile_25=int(adjusted_salary["min"] * 1.1),
            percentile_75=int(adjusted_salary["max"] * 0.9),
            data_sources=["Internal salary database", "Market research", "Industry reports"],
            confidence_score=confidence_score,
            location_adjustment=location_adjustment - 1.0,  # Convert to percentage change
            skill_premium=skills_analysis["total_premium"],
            seniority_multiplier=total_multiplier,
            market_trends=market_trends,
            comparable_positions=comparable_positions
        )
        
    except Exception as e:
        logger.error(f"Error generating salary suggestions: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error generating salary suggestions: {str(e)}")

@app.post("/experience-level-detection", response_model=ExperienceLevelResponse)
async def detect_experience_level(request: ExperienceLevelRequest):
    """
    Automatically detect experience level from job descriptions and requirements.
    
    This endpoint analyzes job descriptions to extract:
    - Required years of experience
    - Seniority level indicators
    - Responsibility level keywords
    - Experience range detection
    """
    try:
        # Combine all text for analysis
        full_text = f"{request.job_title} {request.job_description} {request.requirements or ''}".lower()
        
        # Experience patterns for detecting years of experience
        experience_patterns = [
            r'(\d+)(?:\+|\s*or\s*more|\s*\+)?\s*(?:to\s*(\d+))?\s*years?\s*(?:of\s*)?(?:experience|exp)',
            r'(\d+)(?:\+|\s*or\s*more|\s*\+)?\s*(?:to\s*(\d+))?\s*yrs?\s*(?:of\s*)?(?:experience|exp)',
            r'minimum\s*(?:of\s*)?(\d+)\s*years?',
            r'at\s*least\s*(\d+)\s*years?',
            r'(\d+)\s*(?:to\s*(\d+))?\s*years?\s*(?:in|with|of)',
            r'(\d+)\s*(?:to\s*(\d+))?\s*years?\s*(?:experience|background)',
            r'(\d+)\+\s*years?',
        ...
        for pattern in experience_patterns:
            ...
            for match in matches:
                groups = match.groups()
                min_years = int(groups[0]) if groups[0] else 0
                max_years = int(groups[1]) if len(groups) > 1 and groups[1] else min_years
                ...
                evidence.append(f"Found: '{match.group(0)}'")
        
        # Level indicators for additional context
        level_indicators = {
            'entry': ['entry', 'entry-level', 'junior', 'new grad', 'recent graduate', 'associate', 'trainee', 'intern'],
            'junior': ['junior', '1-2 years', '0-2 years', 'early career', 'associate'],
            'mid': ['mid', 'mid-level', 'intermediate', '3-5 years', '2-5 years', 'experienced'],
            'senior': ['senior', 'sr', '5+ years', '5-8 years', '7+ years', 'expert', 'lead'],
            'lead': ['lead', 'staff', 'principal', '8+ years', '10+ years', 'architect', 'tech lead'],
            'executive': ['director', 'vp', 'chief', 'head of', 'executive', '15+ years', 'management']
        }
        
        # Score each level based on text indicators
        level_scores = {}
        for level, keywords in level_indicators.items():
            ...
            for keyword in keywords:
                if keyword in full_text:
                    ...
                    if keyword not in [e.split("'")[1] for e in evidence if "'" in e]:
                        evidence.append(f"Found indicator: '{keyword}'")
            level_scores[level] = {'score': score, 'indicators': found_indicators}
        
        # Determine experience level based on years and indicators
        def determine_level_from_years(years_data):
            if not years_data:
                return None, 0
            
            # Get the most common range
            min_years = min([y['min'] for y in years_data])
            max_years = max([y['max'] for y in years_data])
            ...
            if avg_years <= 1:
                ...
                return 'Executive', 0.7
        
        # Primary detection from years
        years_level, years_confidence = determine_level_from_years(detected_years)
        
        # Secondary detection from indicators
        indicator_level = max(level_scores.items(), key=lambda x: x[1]['score'])
        indicator_confidence = min(0.8, indicator_level[1]['score'] * 0.15)
        
        # Combine detections with weighted scoring
        final_scores = {}
        
        # Years-based scoring
        if years_level:
            final_scores[years_level] = years_confidence * 0.7  # 70% weight for years
        
        # Indicator-based scoring (30% weight)
        for level, data in level_scores.items():
            level_title = level.title()
            indicator_score = min(0.8, data['score'] * 0.1) * 0.3
            final_scores[level_title] = final_scores.get(level_title, 0) + indicator_score
        
        # Title-based boost
        title_lower = request.job_title.lower()
        if 'senior' in title_lower or 'sr' in title_lower:
            final_scores['Senior'] = final_scores.get('Senior', 0) + 0.3
        elif 'junior' in title_lower or 'jr' in title_lower:
            final_scores['Junior'] = final_scores.get('Junior', 0) + 0.3
        elif 'lead' in title_lower:
            final_scores['Lead'] = final_scores.get('Lead', 0) + 0.3
        elif 'principal' in title_lower or 'staff' in title_lower:
            final_scores['Lead'] = final_scores.get('Lead', 0) + 0.35
        
        # Select the level with highest score
        if final_scores:
            detected_level = max(final_scores.items(), key=lambda x: x[1])
            confidence_score = min(0.95, detected_level[1])
            detected_level = detected_level[0]
        else:
            detected_level = 'Mid'
            confidence_score = 0.4
        
        # Calculate years of experience summary
        years_summary = {
            'detected_ranges': detected_years,
            'min_years': min([y['min'] for y in detected_years]) if detected_years else None,
            'max_years': max([y['max'] for y in detected_years]) if detected_years else None,
            'most_common': detected_years[0] if detected_years else None
        }
        
        # Generate alternative levels with confidence
        level_mapping = {
            'Entry': 0, 'Junior': 1, 'Mid': 2, 'Senior': 3, 'Lead': 4, 'Executive': 5
        }
        
        current_level_index = level_mapping.get(detected_level, 2)
        alternative_levels = []
        
        for level, index in level_mapping.items():
            if level != detected_level:
                # Calculate confidence based on distance from detected level
                distance = abs(index - current_level_index)
                alt_confidence = max(0.1, confidence_score - (distance * 0.2))
                
                # Boost confidence if there's supporting evidence
                if level.lower() in level_scores and level_scores[level.lower()]['score'] > 0:
                    alt_confidence += 0.15
                
                alternative_levels.append({
                    'level': level,
                    'confidence': round(alt_confidence, 2),
                    'reasoning': f"Based on indicators: {level_scores.get(level.lower(), {}).get('indicators', [])}"
                })
        
        # Sort alternatives by confidence
        alternative_levels.sort(key=lambda x: x['confidence'], reverse=True)
        alternative_levels = alternative_levels[:3]  # Top 3 alternatives
        
        # Generate recommendations
        recommendations = []
        
        if confidence_score < 0.6:
            recommendations.append("⚠️ Low confidence in detection - consider adding more specific experience requirements")
        
        if not detected_years:
            recommendations.append("📝 No specific years of experience found - consider adding explicit experience requirements")
        
        if len(detected_years) > 1:
            ranges = [f"{y['min']}-{y['max']} years" for y in detected_years]
            recommendations.append(f"📊 Multiple experience ranges detected: {', '.join(ranges)} - consider consolidating")
        
        if detected_level in ['Lead', 'Executive'] and not any('leadership' in text.lower() or 'management' in text.lower() for text in [request.job_description]):
            recommendations.append("👥 Senior level detected but no leadership keywords found - consider adding management responsibilities")
        
        if detected_level == 'Entry' and 'senior' in request.job_title.lower():
            recommendations.append("🔄 Title-experience mismatch detected - job title suggests senior level but requirements suggest entry level")
        
        return ExperienceLevelResponse(
            detected_level=detected_level,
            confidence_score=round(confidence_score, 2),
            years_of_experience=years_summary,
        ...
        logger.error(f"Error detecting experience level: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error detecting experience level: {str(e)}")

@app.post("/auto-complete", response_model=AutoCompleteResponse)
async def get_auto_complete_suggestions(request: AutoCompleteRequest):
    """
    Provide intelligent auto-complete suggestions while typing job descriptions.
    
    This endpoint analyzes the current context and provides:
    ...
    try:
        ...
        job_title = (request.job_title or "").lower()
        field_type = request.field_type
        
        # Determine context categories based on job title and current text
        context_categories = []
        
        # Technology context detection
        tech_keywords = ['developer', 'engineer', 'programmer', 'frontend', 'backend', 'fullstack', 'react', 'node', 'python', 'javascript']
        if any(keyword in job_title or keyword in context for keyword in tech_keywords):
            context_categories.append('technology')
        
        # Leadership context detection
        leadership_keywords = ['senior', 'lead', 'manager', 'director', 'team', 'manage', 'mentor', 'leadership']
        if any(keyword in job_title or keyword in context for keyword in leadership_keywords):
            context_categories.append('leadership')
        
        # Design context detection
        design_keywords = ['designer', 'ux', 'ui', 'design', 'figma', 'sketch', 'visual', 'creative']
        if any(keyword in job_title or keyword in context for keyword in design_keywords):
            context_categories.append('design')
        
        # Marketing context detection
        marketing_keywords = ['marketing', 'growth', 'campaigns', 'seo', 'content', 'social media', 'analytics']
        if any(keyword in job_title or keyword in context for keyword in marketing_keywords):
            context_categories.append('marketing')
        
        suggestions = []
        
        # Try to get GPT suggestions first (if available)
        if AI_ENABLED and client.api_key:
            try:
                gpt_suggestions = await generate_gpt_autocomplete(context, field_type, job_title)
                
                # Convert GPT suggestions to AutoCompleteSuggestion format
                for i, text in enumerate(gpt_suggestions):
                    suggestions.append(AutoCompleteSuggestion(
                        text=text,
                        type="gpt_suggestion",
                        confidence=0.95 - (i * 0.05),  # High confidence, slightly decreasing
                        category="ai_powered",
                        icon="🤖"
                    ))
                    
            except Exception as e:
                logger.warning(f"GPT autocomplete failed, using fallback: {str(e)}")
        
        # Generate rule-based suggestions as fallback or supplement
        if field_type == "description":
            fallback_suggestions = generate_description_suggestions(context, context_categories, job_title)
        elif field_type == "requirements":
            fallback_suggestions = generate_requirements_suggestions(context, context_categories, job_title)
        elif field_type == "summary":
            fallback_suggestions = generate_summary_suggestions(context, context_categories, job_title)
        else:
            fallback_suggestions = []
        
        # Add fallback suggestions with lower confidence if we have GPT suggestions
        if suggestions:  # We have GPT suggestions
            # Add top 3 fallback suggestions with reduced confidence
            for suggestion in fallback_suggestions[:3]:
                suggestion.confidence = max(0.3, suggestion.confidence - 0.4)
                suggestion.type = "fallback"
                suggestion.icon = "🔧"
                suggestions.append(suggestion)
        else:
            # No GPT suggestions, use all fallback suggestions
        ...
        detected_context = ", ".join(context_categories) if context_categories else "general"
        ai_status = "GPT-Enhanced" if (AI_ENABLED and client.api_key and any(s.type == "gpt_suggestion" for s in suggestions)) else "Rule-Based"
        
        return AutoCompleteResponse(
            suggestions=suggestions,
            context_detected=f"{detected_context} ({ai_status})",
            total_suggestions=len(suggestions)
        )
        
    except Exception as e:
        logger.error(f"Error generating auto-complete suggestions: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error generating auto-complete suggestions: {str(e)}")

def generate_description_suggestions(context: str, categories: list, job_title: str) -> list[AutoCompleteSuggestion]:
    """Generate smart suggestions for job descriptions"""
    suggestions = []
    
    # Common job description starters
    if len(context.strip()) < 20:  # Starting to write
        starters = [
            {
                "text": "We are seeking a talented and motivated",
                "type": "phrase",
                "confidence": 0.9,
                "category": "opening",
                "icon": "🚀"
            ...
            {
                "text": "Join our dynamic team as a",
                "type": "phrase", 
                "confidence": 0.85,
                "category": "opening",
                "icon": "👥"
            ...
            {
                "text": "We're looking for an experienced",
                "type": "phrase",
                "confidence": 0.8,
                "category": "opening", 
                "icon": "🎯"
        ...
        suggestions.extend([AutoCompleteSuggestion(**s) for s in starters])
    
    # Technology-specific suggestions
    if 'technology' in categories:
        tech_suggestions = [
            {
                "text": "to join our engineering team and contribute to building scalable web applications.",
                "type": "completion",
                "confidence": 0.85,
            ...
            {
                "text": "You will be responsible for developing and maintaining high-quality software solutions.",
                "type": "phrase",
                "confidence": 0.8,
                "category": "responsibility",
                "icon": "💻"
            ...
            {
                "text": "The ideal candidate will have experience with modern web technologies and best practices.",
                "type": "phrase",
                "confidence": 0.75,
            ...
            {
                "text": "Work collaboratively with cross-functional teams including design, product, and QA.",
                "type": "phrase",
                "confidence": 0.7,
                "category": "responsibility",
                "icon": "🤝"
        ...
        suggestions.extend([AutoCompleteSuggestion(**s) for s in tech_suggestions])
    
    # Leadership-specific suggestions
    if 'leadership' in categories:
        leadership_suggestions = [
            {
                "text": "Lead and mentor a team of developers while driving technical excellence.",
                "type": "phrase",
                "confidence": 0.85,
                "category": "responsibility",
                "icon": "👨‍💼"
            ...
            {
                "text": "Provide technical guidance and architectural decisions for complex projects.",
                "type": "phrase",
                "confidence": 0.8,
                "category": "responsibility",
                "icon": "🏗️"
            ...
            {
                "text": "Foster a collaborative environment and promote best practices across the team.",
                "type": "phrase",
                "confidence": 0.75,
                "category": "responsibility",
                "icon": "🌟"
        ...
        suggestions.extend([AutoCompleteSuggestion(**s) for s in leadership_suggestions])
    
    # Context-aware completions based on current text
    if "responsible" in context and not "for" in context:
        suggestions.append(AutoCompleteSuggestion(
            text="for developing and implementing",
            type="completion",
            confidence=0.9,
            category="responsibility",
            icon="📋"
    ...
    if "experience" in context and not "with" in context:
        suggestions.append(AutoCompleteSuggestion(
            text="with modern technologies and frameworks",
            type="completion",
            confidence=0.85,
            category="requirement",
            icon="🎓"
    ...
    if "work" in context and not "with" in context:
        suggestions.append(AutoCompleteSuggestion(
            text="with a collaborative team in a fast-paced environment",
            type="completion",
            confidence=0.8,
...
def generate_requirements_suggestions(context: str, categories: list, job_title: str) -> list[AutoCompleteSuggestion]:
    """Generate smart suggestions for job requirements"""
    suggestions = []
    ...
    if 'technology' in categories:
        tech_requirements = [
            {
                "text": "Strong proficiency in JavaScript, HTML, and CSS",
                "type": "bullet_point",
                "confidence": 0.9,
                "category": "technical",
                "icon": "💻"
            ...
            {
                "text": "Experience with React, Vue.js, or Angular",
                "type": "bullet_point",
                "confidence": 0.85,
                "category": "technical",
                "icon": "⚛️"
            ...
            {
                "text": "Understanding of RESTful APIs and web services",
                "type": "bullet_point",
                "confidence": 0.75,
                "category": "technical",
                "icon": "🌐"
            ...
            {
                "text": "Knowledge of testing frameworks and methodologies",
                "type": "bullet_point",
                "confidence": 0.7,
                "category": "technical",
                "icon": "🧪"
        ...
        suggestions.extend([AutoCompleteSuggestion(**s) for s in tech_requirements])
    ...
    general_requirements = [
        ...
        {
            "text": "Strong communication and collaboration abilities",
            "type": "bullet_point",
            "confidence": 0.8,
            "category": "soft_skill",
            "icon": "💬"
        ...
        {
            "text": "Ability to work independently and manage multiple priorities",
            "type": "bullet_point",
            "confidence": 0.7,
            "category": "soft_skill",
            "icon": "⚖️"
    ...
    suggestions.extend([AutoCompleteSuggestion(**s) for s in general_requirements])
    
    return suggestions

def generate_summary_suggestions(context: str, categories: list, job_title: str) -> list[AutoCompleteSuggestion]:
    """Generate smart suggestions for job summaries"""
    suggestions = []
    
    summary_suggestions = [
        {
            "text": "Exciting opportunity to work with cutting-edge technologies",
            "type": "phrase",
            "confidence": 0.85,
            "category": "benefit",
            "icon": "🚀"
        ...
        {
            "text": "Competitive salary and comprehensive benefits package",
            "type": "phrase",
            "confidence": 0.8,
            "category": "benefit",
            "icon": "💰"
        ...
        {
            "text": "Flexible work arrangements and professional development opportunities",
            "type": "phrase",
            "confidence": 0.75,
            "category": "benefit",
            "icon": "📈"
        ...
        {
            "text": "Join a growing company with a strong engineering culture",
            "type": "phrase",
            "confidence": 0.7,
            "category": "culture",
            "icon": "🏢"
    ...
    suggestions.extend([AutoCompleteSuggestion(**s) for s in summary_suggestions])
...
async def get_ai_service_status():
    ...
    try:
        ...
        return {
            ...
            "performance": {
                "enhanced_accuracy": status["enabled"],
                "context_awareness": status["enabled"],
                "multi_language_support": status["enabled"]
            },
            "usage_info": {
                "gpt_integration": "✅ Active" if status["enabled"] else "❌ Fallback Mode",
                "recommendation": "Add OpenAI API key to .env file for enhanced AI features" if not status["api_key_configured"] else "AI service fully operational"
            }
        }
...
@app.post("/ai/test")
async def test_ai_integration(request: dict):
    """Test AI integration with sample job data"""
    try:
        # Sample test data
        test_job = {
            "title": request.get("title", "Senior Software Engineer"),
            "description": request.get("description", "We are looking for an experienced software engineer to join our team and work on cutting-edge web applications using React and Node.js."),
        ...
        try:
            summary = await generate_ai_summary(test_job, "standard")
        ...
        try:
            analysis = await analyze_ai_requirements(test_job["title"], test_job["description"])
            results["requirements_analysis"] = {
                "status": "✅ Success",
                "sample_skills": analysis.get("core_skills", [])[:5] if isinstance(analysis.get("core_skills"), list) else []
            }
        except Exception as e:
            results["requirements_analysis"] = {
                "status": "❌ Failed",
        ...
        try:
            suggestions = await generate_ai_suggestions(f"{test_job['title']} - {test_job['description']}", "skills")
            results["skills_suggestions"] = {
                "status": "✅ Success",
                "sample_suggestions": suggestions[:5] if isinstance(suggestions, list) else []
            }
        except Exception as e:
            results["skills_suggestions"] = {
                "status": "❌ Failed",
        ...
        return {
            ...
            "recommendation": "All AI features tested successfully! 🎉" if all("✅" in r.get("status", "") for r in results.values()) else "Some AI features need attention. Check API key configuration."
        }
...
from enhanced_cv_analyzer import GPTCVAnalyzer, EnhancedCVAnalysis
...
@app.post("/cvs/extract-text")
async def extract_cv_text(file: UploadFile = FastAPIFile(...)):
    """Extract text content from uploaded CV file (PDF, DOC, DOCX)"""
    try:
...
@app.post("/cvs/analyze-content")
async def analyze_cv_content(request: CVAnalysisRequest):
    """Enhanced CV analysis using advanced contextual reasoning"""
    try:
        # Initialize the enhanced analyzer
        analyzer = GPTCVAnalyzer()
        
        # Perform comprehensive analysis
        analysis_result = analyzer.analyze_cv_comprehensive(
            cv_text=request.cv_text,
            job_description=request.job_description
        )
        
        # Convert to dictionary for JSON response - Include ALL data from enhanced analyzer
        response_data = {
            "candidate_name": analysis_result.candidate_name,
            "primary_role": analysis_result.primary_role,
            "experience_years": analysis_result.experience_years,
            "technical_skills": [
                {
                    "skill_name": skill.skill_name,
                    "name": skill.skill_name,  # Backward compatibility
                    "evidence": skill.evidence_text,
                    "evidence_text": skill.evidence_text,  # Frontend expects this field
                    "evidence_type": skill.evidence_type.value,
                    "proficiency_score": skill.proficiency_score,
                    "confidence": skill.proficiency_score,  # Backward compatibility
                    "transferability_score": skill.transferability_score,
                    "depth_indicators": skill.depth_indicators,
                    "context": skill.context,
                    # Frontend expects these additional fields from enhanced analyzer
                    "evidence_sentences": getattr(skill, 'evidence_sentences', [skill.evidence_text]) if hasattr(skill, 'evidence_sentences') else [skill.evidence_text],
                    "skill_weight": getattr(skill, 'skill_weight', 10.0) if hasattr(skill, 'skill_weight') else 10.0,
                    "weighted_score": getattr(skill, 'weighted_score', skill.proficiency_score * 0.1) if hasattr(skill, 'weighted_score') else skill.proficiency_score * 0.1,
                    "portfolio_links": getattr(skill, 'portfolio_links', []) if hasattr(skill, 'portfolio_links') else []
                }
            ...
            "soft_skills": [
                {
                    ...
                    "confidence": skill.confidence_score,  # Backward compatibility
                    "supporting_context": skill.supporting_context,
                    "evidence": skill.supporting_context,  # Frontend expects this
                    "level": "Strong" if skill.confidence_score >= 80 else "Good" if skill.confidence_score >= 60 else "Developing"
                }
                for skill in analysis_result.soft_skills
            ],
            "portfolio_links": analysis_result.portfolio_links,
            "certifications": analysis_result.certifications,
            "career_progression": analysis_result.career_progression,
            "strengths": analysis_result.strengths,
            "improvement_suggestions": analysis_result.improvement_suggestions,
            "ai_insights": analysis_result.ai_insights,
            "analysis_type": "enhanced_contextual",
            # Include the job_fit_analysis properly converted to dict
            "job_fit_analysis": {
                "overall_fit": analysis_result.job_fit_analysis.overall_fit,
                "requirement_breakdown": analysis_result.job_fit_analysis.requirement_breakdown,
                "strengths_for_role": analysis_result.job_fit_analysis.strengths_for_role,
                "gaps_to_address": analysis_result.job_fit_analysis.gaps_to_address,
                "hiring_decision": analysis_result.job_fit_analysis.hiring_decision,
                "recommendation": analysis_result.job_fit_analysis.recommendation,
                "interview_focus_areas": analysis_result.job_fit_analysis.interview_focus_areas,
                "overall_score": analysis_result.job_fit_analysis.overall_score
            } if analysis_result.job_fit_analysis else None
        }
        ...
        try:
            return {
                "candidate_name": extract_name_from_text(request.cv_text),
                "primary_role": "Role not determined",
                "experience_years": extract_experience_years(request.cv_text),
                "technical_skills": [{"name": skill, "proficiency_score": 50} for skill in extract_skills_basic(request.cv_text)],
                "soft_skills": [],
                "strengths": [],
                "improvement_suggestions": ["Enhanced analysis unavailable"],
                "ai_insights": f"Enhanced analysis failed: {str(e)}. Using basic fallback.",
                "analysis_type": "basic_fallback"
            }
        except Exception as fallback_error:
            raise HTTPException(status_code=500, detail=f"All analysis methods failed: {str(fallback_error)}")

@app.post("/cvs/match-job")
async def match_cv_with_job(request: JobMatchRequest):
    """Advanced CV-Job matching with transferable skills consideration"""
    try:
        # Initialize the enhanced analyzer
        analyzer = GPTCVAnalyzer()
        
        # First analyze the CV
        cv_analysis = analyzer.analyze_cv_comprehensive(
            cv_text=request.cv_text,
            job_description=request.job_description
        )
        
        # Perform job matching
        match_result = analyzer.match_with_job_description(cv_analysis, request.job_description)
        
        return {
            "cv_analysis": {
                "candidate_name": cv_analysis.candidate_name,
                "primary_role": cv_analysis.primary_role,
                "experience_years": cv_analysis.experience_years,
                "top_skills": [skill.skill_name for skill in cv_analysis.technical_skills[:5]],
                "strengths": cv_analysis.strengths
            },
            "job_match": match_result,
            "recommendations": _generate_match_recommendations(cv_analysis, match_result),
            "analysis_type": "enhanced_job_matching"
        }
...
def _generate_match_recommendations(cv_analysis: EnhancedCVAnalysis, match_result: Dict) -> List[str]:
    """Generate specific recommendations for improving job match"""
    recommendations = []
    ...
    if transferable:
        for skill_match in transferable[:2]:
            recommendations.append(
                f"Highlight your {skill_match['cv_skill']} experience as it's {skill_match['transferability_score']}% transferable to {skill_match['jd_skill']}"
            )
    
    # CV improvement suggestions
    if cv_analysis.improvement_suggestions:
        recommendations.extend(cv_analysis.improvement_suggestions[:2])
    
    return recommendations

def extract_name_from_text(text: str) -> str:
    """Basic name extraction from CV text"""
    lines = text.split('\n')[:5]  # Check first 5 lines
    for line in lines:
        line = line.strip()
        if line and not any(keyword in line.lower() for keyword in ['email', 'phone', 'address', 'linkedin']):
            if len(line.split()) <= 4 and len(line) > 3:  # Likely a name
                return line
...
def extract_experience_years(text: str) -> int:
    ...
    job_keywords = ['worked at', 'employed', 'position', 'role', 'job']
    job_count = sum(1 for keyword in job_keywords if keyword in text.lower())
    return max(1, job_count * 2)  # Rough estimate

def extract_skills_basic(text: str) -> List[Dict]:
    """Basic skill extraction without AI"""
    # Common programming languages and technologies
    tech_skills = [
        'Python', 'JavaScript', 'Java', 'C++', 'C#', 'React', 'Angular', 'Vue',
        'Node.js', 'Django', 'Flask', 'SQL', 'MongoDB', 'PostgreSQL', 'MySQL',
        'HTML', 'CSS', 'TypeScript', 'Git', 'Docker', 'AWS', 'Azure', 'Linux'
    ]
    ...
    for skill in tech_skills:
        if skill.lower() in text_lower:
            ...
            import re
            pattern = rf'.{{0,50}}{re.escape(skill.lower())}.{{0,50}}'
            ...
            found_skills.append({
                "name": skill,
                "level": "Mentioned",
                "evidence": f"Found in CV: {context}",
                "context": context,
                "proficiency_score": 50,  # Default score
                "projects": []
            })
    
    return found_skills[:20]  # Limit to 20 skills (increased from 10 for better CV analysis)

# Server startup code
if __name__ == "__main__":
    import uvicorn
    logger.info(f"Loaded skills taxonomy with {SKILLS_TAXONOMY['metadata']['total_skills']} skills")
    logger.info("Starting CV2 Gateway API server...")
    uvicorn.run(
        "main:app", 
        host="0.0.0.0", 
        port=8000, 
