...
interface AnalysisResult {
  ...
  job_comparison?: any;  // Add this property
  ai_summary: string;
  error?: string;  // Add this property for error cases
  job_fit_analysis?: {
    overall_fit: string;
      ...
      return {
        ...
        job_fit_analysis: {
          ...
          requirement_breakdown: job.requirements.map((req: any) => {
            ...
            const score = hasSkill ? (matchingSkill?.proficiency_score || 75) : 30;
            ...
            return {
              ...
              gap_analysis: !hasSkill ? `Consider training or certification in ${req.skill}` : score < 70 ? "Could benefit from more experience" : null
            };
          }),
                  ...
                  <div key={index} className="space-y-6">
                    ...
                    {expandedResults[index] && (
                      <div className="space-y-6">
                        ...
                        <Card padding="xl">
                          ...
                          <div className="space-y-8">
                            ...
                            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-6 border border-indigo-200 rounded-xl">
                              <h3 className="flex items-center mb-4 font-bold text-indigo-800 text-lg">
                              ...
                              <div className="space-y-4">
                                ...
                                {result.technical_skills && result.technical_skills.length > 0 && (
                                  <div>
                                    <h4 className="mb-3 font-semibold text-gray-800">Languages & Frameworks:</h4>
                                    <div className="flex flex-wrap gap-2 mb-4">
                                      {result.technical_skills.slice(0, 8).map((skill, idx) => (
                                        <span key={idx} className={`px-3 py-1 rounded-full text-sm font-medium ${
                                          skill.proficiency_score >= 80 ? 'bg-green-100 text-green-800' :
                                          skill.proficiency_score >= 60 ? 'bg-yellow-100 text-yellow-800' :
                                ...
                                <div className="space-y-4">
                                  <h4 className="mb-3 font-semibold text-gray-800">Complete Technical Skills Profile:</h4>
                                  
                                  {/* All Technical Skills with Evidence */}
                                  <div className="gap-3 grid">
                                    {result.technical_skills?.map((skill, idx) => (
                                      <div key={idx} className="flex justify-between items-center bg-white p-3 border rounded-lg">
                                        <div className="flex-1">
                                          <span className="font-medium text-gray-900 capitalize">{skill.name}</span>
                                          <div className="mt-1 text-gray-600 text-sm">
                                            Evidence: "{skill.evidence.slice(0, 80)}..."
                                          </div>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                          <span className={`px-2 py-1 rounded text-xs font-medium ${
                                            skill.proficiency_score >= 80 ? 'bg-green-100 text-green-700' :
                                            skill.proficiency_score >= 60 ? 'bg-yellow-100 text-yellow-700' :
                                            skill.proficiency_score >= 40 ? 'bg-blue-100 text-blue-700' :
                                          ...
                                          <span className="text-gray-500 text-xs">{skill.proficiency_score}%</span>
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                            ...
                            <div className="bg-gradient-to-r from-green-50 to-teal-50 p-6 border border-green-200 rounded-xl">
                              <h3 className="flex items-center mb-4 font-bold text-green-800 text-lg">
                                <i className="mr-3 fas fa-star"></i>
                                Strengths for This Role
                              </h3>
                              <ul className="space-y-3 text-gray-700">
                                <li className="flex items-start">
                                  <i className="mt-1.5 mr-3 text-green-600 text-sm fas fa-check-circle"></i>
                                  <span>Strong expertise in {result.primary_role?.toLowerCase() || 'frontend'} technologies with proven ability to deliver high-quality, responsive applications.</span>
                                </li>
                                <li className="flex items-start">
                                  <i className="mt-1.5 mr-3 text-green-600 text-sm fas fa-check-circle"></i>
                                  <span>Track record of {result.experience_years}+ years experience with scalability enhancements and modern development practices.</span>
                                </li>
                                {result.soft_skills?.find(s => s.name.toLowerCase().includes('leadership')) && (
                                  <li className="flex items-start">
                                    <i className="mt-1.5 mr-3 text-green-600 text-sm fas fa-check-circle"></i>
                                    <span>Experience mentoring junior developers and leading team initiatives.</span>
                                  </li>
                                )}
                                <li className="flex items-start">
                                  <i className="mt-1.5 mr-3 text-green-600 text-sm fas fa-check-circle"></i>
                                  <span>Effective collaborator with proven ability to work in team environments and deliver results.</span>
                                </li>
                                <li className="flex items-start">
                                  <i className="mt-1.5 mr-3 text-green-600 text-sm fas fa-check-circle"></i>
                                  <span>Consistent engagement in continuous learning and adoption of emerging technologies.</span>
                                </li>
                              </ul>
                            </div>
                                ...
                                <ul className="space-y-2 text-gray-700">
                                  {result.improvement_suggestions.map((suggestion, idx) => (
                                    <li key={idx} className="flex items-start">
                                      <i className="fa-arrow-right mt-1.5 mr-3 text-orange-600 text-sm fas"></i>
                            ...
                            <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 border border-purple-200 rounded-xl">
                              <h3 className="flex items-center mb-4 font-bold text-purple-800 text-lg">
                              ...
                              <div className="space-y-4">
                                <div className="flex items-center">
                                  <span className={`px-4 py-2 rounded-full font-bold text-lg ${
                                    result.experience_years >= 5 ? 'bg-green-100 text-green-800' :
                                    result.experience_years >= 3 ? 'bg-yellow-100 text-yellow-800' :
                                    'bg-gray-100 text-gray-800'
                                  }`}>
                                    {result.experience_years >= 5 ? 'Strong Hire' : 
                                     result.experience_years >= 3 ? 'Consider' : 'Entry Level'}
                                  </span>
                                </div>
                                <p className="text-gray-700 leading-relaxed">
                                  Candidate demonstrates strong technical expertise, collaborative mindset, and growth potential. 
                                  Brings {result.experience_years} years of experience and proven ability to work with modern technologies, 
                                  making them well-suited for this {job.title.toLowerCase()} role.
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-r from-cyan-50 to-blue-50 p-6 border border-cyan-200 rounded-xl">
                              ...
                              <ul className="space-y-3 text-gray-700">
                                <li className="flex items-start">
                                  ...
                                  <span>Deep dive into {result.technical_skills?.slice(0, 2).map(s => s.name).join(' and ')} experience and complex application architecture.</span>
                                </li>
                                <li className="flex items-start">
                                  <i className="mt-1.5 mr-3 text-cyan-600 text-sm fas fa-question-circle"></i>
                                  ...
                                  {result.soft_skills.map((skill, skillIndex) => (
                                    <div key={skillIndex} className="bg-white p-4 border rounded-lg">
                                      <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${
                                          skill.confidence_score >= 80 ? 'bg-green-100 text-green-700' :
                                          skill.confidence_score >= 60 ? 'bg-yellow-100 text-yellow-700' :
                                          'bg-gray-100 text-gray-700'
                                        }`}>
                                          {skill.confidence_score}%
                                        </span>
                                      </div>
                                      <p className="mb-2 text-gray-600 text-sm">{skill.supporting_context}</p>
                                      <div className="text-gray-500 text-xs">
                                        Evidence: {skill.evidence_sentences.length} instance(s)
                                      </div>
                                    </div>
                                  ))}
                                </div>
                            ...
                            {result.certifications && result.certifications.length > 0 && (
                              <div className="bg-yellow-50 p-6 border border-yellow-200 rounded-xl">
                                <h3 className="flex items-center mb-4 font-semibold text-yellow-700">
                                  <i className="mr-2 fas fa-certificate"></i>
                                  Certifications ({result.certifications.length})
                                </h3>
                                <div className="gap-4 grid md:grid-cols-2">
                                  {result.certifications.map((cert, certIndex) => (
                                    <div key={certIndex} className="bg-white p-4 border rounded-lg">
                                      <h4 className="font-medium text-gray-900">{cert.name || 'Certification'}</h4>
                                      <p className="text-gray-600 text-sm">{cert.issuer || 'Unknown issuer'}</p>
                                      <p className="text-gray-500 text-xs">{cert.date || 'Date not specified'}</p>
                                    </div>
                                  ))}
                                </div>
                              </div>
                        ...
                        <Card padding="xl">
                          <SectionHeader 
                            title={`🎯 CV vs ${job.title} Comparison`}
                            subtitle="Detailed assessment of candidate fit against this specific role"
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 border border-blue-200 rounded-xl">
                              ...
                              <p className="text-gray-700 leading-relaxed">{result.job_fit_analysis?.overall_fit || 'Analysis not available'}</p>
                            </div>
                            ...
                            <div className="bg-gray-50 p-6 border border-gray-200 rounded-xl">
                              <h3 className="flex items-center mb-4 font-semibold text-gray-700">
                                <i className="mr-2 fa-list-check fas"></i>
                                Job Requirements Analysis
                              </h3>
                              <div className="space-y-4">
                                {result.job_fit_analysis?.requirement_breakdown?.map((req: any, idx: number) => (
                                  <div key={idx} className="bg-white p-4 border border-gray-200 rounded-lg">
                                    <div className="flex justify-between items-start mb-2">
                                      <div className="flex-1">
                                        <h4 className="font-semibold text-gray-900">{req.requirement}</h4>
                                        <p className="text-gray-600 text-sm">Required: {req.required_level} • Candidate: {req.candidate_level}</p>
                                      </div>
                                      <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                                        req.match_score >= 80 ? 'bg-green-100 text-green-700' :
                                        req.match_score >= 60 ? 'bg-yellow-100 text-yellow-700' : 
                                        'bg-red-100 text-red-700'
                                      }`}>
                                        {req.match_score}%
                                      </div>
                                    </div>
                                    <p className="mb-2 text-gray-600 text-sm">{req.evidence}</p>
                                    {req.match_score < 70 && (
                                      <div className="bg-orange-50 p-2 border border-orange-200 rounded text-orange-700 text-sm">
                            ...
                            <div className="gap-6 grid md:grid-cols-2">
                              ...
                              <div className="bg-green-50 p-6 border border-green-200 rounded-xl">
                                ...
                                <ul className="space-y-2">
                                  {result.job_fit_analysis?.strengths_for_role?.map((strength: string, idx: number) => (
                                    <li key={idx} className="flex items-start">
                                      <i className="mt-1 mr-2 text-green-600 text-xs fas fa-plus"></i>
                                      <span className="text-gray-700 text-sm">{strength}</span>
                                    </li>
                                  )) || <li className="text-gray-500 text-sm">No strengths identified for this role</li>}
                                </ul>
                              </div>
                              ...
                              <div className="bg-orange-50 p-6 border border-orange-200 rounded-xl">
                                ...
                                <ul className="space-y-2">
                                  {result.job_fit_analysis?.gaps_to_address?.map((gap: string, idx: number) => (
                                    <li key={idx} className="flex items-start">
                                      <i className="mt-1 mr-2 text-orange-600 text-xs fas fa-arrow-up"></i>
                                      <span className="text-gray-700 text-sm">{gap}</span>
                                    </li>
                                  )) || <li className="text-gray-500 text-sm">No specific gaps identified</li>}
                                </ul>
                              </div>
                            </div>
                            ...
                            <div className="bg-purple-50 p-6 border border-purple-200 rounded-xl">
                              ...
                              <p className="mb-4 text-gray-700">{result.job_fit_analysis?.recommendation || 'Recommendation not available'}</p>
                              <div className="flex items-center space-x-3">
                                <span className="text-gray-600 text-sm">Decision:</span>
                                <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getHiringDecisionColor(result.job_fit_analysis?.hiring_decision || 'maybe')}`}>
                                  {getHiringDecisionText(result.job_fit_analysis?.hiring_decision || 'maybe')}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="bg-indigo-50 p-6 border border-indigo-200 rounded-xl">
                              <h3 className="flex items-center mb-4 font-semibold text-indigo-700">
                                <i className="mr-2 fas fa-comments"></i>
                                Interview Focus Areas
                              </h3>
                              <div className="gap-3 grid md:grid-cols-2">
                                {result.job_fit_analysis?.interview_focus_areas?.map((area: string, idx: number) => (
                                  <div key={idx} className="bg-white p-3 border border-indigo-200 rounded-lg">
                                    <div className="flex items-start">
                                      <i className="mt-1 mr-2 text-indigo-600 text-xs fas fa-question-circle"></i>
                                      <span className="text-gray-700 text-sm">{area}</span>
                                    </div>
                                  </div>
                                )) || <div className="text-gray-500 text-sm">No specific interview focus areas identified</div>}
                              </div>
                            </div>
                          </div>
                        </Card>
